...
def primeiro_cliente(request):
    orcamentos = Orcamento.objects.get(id=1)
    return render(request, 'mp_orcamento/primeiro_cliente.html', {'orcamentos': orcamentos})
...
def cliente_estatisticas(request):
    ...
    maior_custo = 0
    menor_custo = 999999999999
    cliente_maior_orcamento = None
    cliente_menor_orcamento = None
    somatorio_custo_total = 0
    for orcamento in orcamentos:
        somatorio = 0
        for peca in Peca.objects.filter(orcamento=orcamento):
        ...
        somatorio_custo_total += orcamento.custo_total
        if orcamento.custo_total >= maior_custo:
                cliente_maior_orcamento = orcamento
                maior_custo = orcamento.custo_total
        if orcamento.custo_total <= menor_custo:
                cliente_menor_orcamento = orcamento
                menor_custo = orcamento.custo_total
    return render(request, 'mp_orcamento/cliente_estatisticas.html', {'quantidade': quantidade,
                                                                      'cliente_maior_orcamento': cliente_maior_orcamento,
                                                                      'cliente_menor_orcamento': cliente_menor_orcamento
                                                                      })
